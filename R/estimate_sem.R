...
#' Estimate a one-factor structural equation
#'
...
#'      terminated; see {"code"} from [stats::nlm()]}
#'     \item{nlm_iterations}{The number of [stats::nlm()] iterations performed}
...
#' Create initial vector of parameter values for estimating a joint SEM
#'
...
#'   maximization.
initalize_phi <- function(data0, endpoints, categorical = c(), treatment,
                          gamma = 2) {
  ...
  for (p in 1:length(endpoints)) {
    if (endpoints[p] %in% categorical) {
      ...
      if (n_thresholds[endpoints[p]] > 0) {
        ...
        cum_props_p <- cumsum(
          prop.table(table(data0[data0[[treatment]] == 0, endpoints[p]])))
      ...
      lambda[p] <- 1/gamma * (
        mean(data0[data0[[treatment]] == 1, endpoints[p]]) - nu[p])

      logtheta_p <- log(
        max(
          var(data0[data0[[treatment]] == 0, endpoints[p]]) - lambda[p]^2,
          1e-5
          )
        )
...
ll_sem <- function(phi, data0, endpoints, categorical, treatment,
                   .names = NULL) {
  ...
  if (q > 0) {
    ...
    for (p in 1:length(categorical)) {
      ...
      logdiffs <-
        phi[grep(paste0("logdiffa_", categorical[p], "_"), names(phi))]
...
estimate_effects <- function(model, risk_difference = c(), sandwich = FALSE) {
  ...
  if (length(risk_difference) > 0) {
    ...
    names(ests) <- c(
      endpoints,
      paste0(risk_difference, "_RD"),
      paste0(risk_difference, "_SD")
      )
...
#' Estimate conditional means from a joint SEM
#'
#' @param model An object of class [joint_sem]
#' @param sandwich Logical indicator to use the sandwich variance estimator
#' @inherit joint_sem examples
